...
class TradingStrategy():
    ...
    def __init__(self, wallet_a, wallet_b, frac_kelly, trade_threshold):
        ...
        self.wallet_a = {'mean_reversion': wallet_a, 'pure_forcasting': wallet_a,
                         'ensemble_with_llm_mean_reversion': wallet_a}
        self.wallet_b = {'mean_reversion': wallet_b, 'pure_forcasting': wallet_b,
                         'ensemble_with_llm_mean_reversion': wallet_b}
        # Track profit/loss, wins/losses, and total gains/losses for each strategy
        self.total_profit_or_loss = {
            'mean_reversion': 0, 'pure_forcasting': 0, 'ensemble_with_llm_mean_reversion': 0}
        self.num_trades = {
            'mean_reversion': 1, 'pure_forcasting': 1, 'ensemble_with_llm_mean_reversion': 1}
        self.num_wins = {'mean_reversion': 0, 'pure_forcasting': 0,
                         'ensemble_with_llm_mean_reversion': 0}
        self.num_losses = {
            'mean_reversion': 0, 'pure_forcasting': 0, 'ensemble_with_llm_mean_reversion': 0}
        self.total_gains = {
            'mean_reversion': 0, 'pure_forcasting': 0, 'ensemble_with_llm_mean_reversion': 0}
    ...
    def calculate_profit_for_signals(self, curr_ratio, next_ratio):
        ...
        for direction in ['buy_currency_a', 'sell_currency_a']:
            ...
            if direction == 'buy_currency_a':
                profit = self.fixed_position_size * \
                    (next_ratio - curr_ratio) / next_ratio
            else:  # sell_currency_a
                profit = self.fixed_position_size * \
                    (curr_ratio - next_ratio) / next_ratio
    ...
    def win_loss_ratio(self, strategy_name):
        ...
        total_trades = self.num_wins[strategy_name] + \
            self.num_losses[strategy_name]
        ...
        avg_gain = (self.total_gains[strategy_name] /
                    self.num_wins[strategy_name]) if self.num_wins[strategy_name] else 1.0
        avg_loss = (self.total_losses[strategy_name] / self.num_losses[strategy_name]
                    ) if self.num_losses[strategy_name] else 1.0
    ...
    def calculate_profit(self, strategy_name, trade_direction, bid_price, ask_price, f_i, use_kelly, enable_transaction_costs):
        ...
        if self.open_positions[strategy_name]['type'] is not None:
            ...
            current_position_type = 'long' if self.open_positions[
                strategy_name]['type'] == 'long' else 'short'
            ...
            if new_position_type is not None and new_position_type != current_position_type:
                profit_in_base_currency += self.close_position(
                    strategy_name, sell_price, buy_price)
            else:
        ...
        if trade_direction != 'no_trade':
            ...
            total_value_in_a = self.wallet_a[strategy_name] + \
                (self.wallet_b[strategy_name] / buy_price)
    ...
    def close_position(self, strategy_name, sell_price, buy_price):
        ...
        if position['type'] == 'long':
            ...
            profit_in_base_currency = position['size_a'] * \
                (sell_price - position['entry_ratio']) / buy_price
            ...
            profit_in_base_currency = position['size_a'] * \
                (position['entry_ratio'] - buy_price) / buy_price

        # Reset position tracking
        self.open_positions[strategy_name] = {
            'type': None, 'size_a': 0, 'size_b': 0, 'entry_ratio': 0}
    ...
    def close_position_signal(self, strategy_name, curr_ratio, next_ratio):
        ...
        if position['type'] == 'long':
            ...
            profit = self.fixed_position_size * \
                (curr_ratio - position['entry_ratio']) / curr_ratio
            ...
            profit = self.fixed_position_size * \
                (position['entry_ratio'] - curr_ratio) / curr_ratio

        # Reset position tracking
        self.open_positions_signal[strategy_name] = {
            'type': None, 'size_a': 0, 'size_b': 0, 'entry_ratio': 0}
    ...
    def get_strategy_signals(self, base_ratio_change, predicted_ratio_change, curr_ratio, next_ratio, llm_sentiment):
        ...
        for strategy_name in strategy_names:
            trade_direction = self.determine_trade_direction(
                strategy_name, base_ratio_change, predicted_ratio_change, llm_sentiment)
            if trade_direction == 'buy_currency_a':
                signals[strategy_name] = self.fixed_position_size * \
                    (next_ratio - curr_ratio) / next_ratio
            elif trade_direction == 'sell_currency_a':
                signals[strategy_name] = self.fixed_position_size * \
                    (curr_ratio - next_ratio) / next_ratio
    ...
    def display_profit_per_trade(self):
        ...
        for strategy_name in self.total_profit_or_loss.keys():
            ...
            if total_trades > 0:
                avg_profit = self.total_profit_or_loss[strategy_name] / \
                    total_trades
            else:
                avg_profit = 0
    ...
    def get_weighted_trade_direction(self, base_percentage_increase, predicted_percentage_increase, llm_sentiment):
        ...
        mean_rev_dir = self.determine_trade_direction(
            "mean_reversion", base_percentage_increase, predicted_percentage_increase, 0)
        forecast_dir = self.determine_trade_direction(
            "pure_forcasting", base_percentage_increase, predicted_percentage_increase, 0)
        llm_dir = self.determine_trade_direction(
            "llm", base_percentage_increase, predicted_percentage_increase, llm_sentiment)
    ...
    def simulate_trading_with_strategies(self, actual_rates, predicted_rates, bid_prices, ask_prices, llm_sentiments, use_kelly=True, enable_transaction_costs=False):
        ...
        for i in range(2, split_idx):
            ...
            if prev_ratio != 0 and curr_ratio != 0:
                ...
                predicted_percentage_increase = (
                    (predicted_next_ratio - curr_ratio) / curr_ratio) * 100
                base_percentage_increase = (
                    (curr_ratio - prev_ratio) / prev_ratio) * 100
            else:
                print("Skipping iteration due to zero division risk.")

            signals = self.get_strategy_signals(
                base_percentage_increase, predicted_percentage_increase, curr_ratio, actual_next_ratio, llm_sentiment)
            ...
            profit, _ = self.calculate_profit_for_signals(
                curr_ratio, actual_next_ratio)
            max_cumulative_profit += profit
            feature_vector = [mean_reversion_cumulative_profit,
                              forecast_cumulative_profit, llm_sentiment_cumulative_profit]
        ...
        self.train_linear_regression(
            historical_data, self.xgb_model_llm_mean_reversion)

        self.mean_reversion_coeff_llm = self.xgb_model_llm_mean_reversion.feature_importances_[
            0]
        self.forecasting_coeff_llm = self.xgb_model_llm_mean_reversion.feature_importances_[
            1]
        self.llm_sentiment_coeff = self.xgb_model_llm_mean_reversion.feature_importances_[
            2]
        ...
        for i in range(split_idx + 2, len(actual_rates) - 1):
            ...
            trade_direction = self.get_weighted_trade_direction(
                base_percentage_increase, predicted_percentage_increase, llm_sentiment)
            ...
            profit = self.calculate_profit(
                strategy_name, trade_direction, bid_price, ask_price, f_i, use_kelly, enable_transaction_costs)
        ...
        for strategy_name in strategy_names:
            for i in range(split_idx + 2, len(actual_rates) - 1):
                ...
                # Determine the trade direction based on the strategy and ratio changes
                trade_direction = self.determine_trade_direction(
                    strategy_name, base_percentage_increase, predicted_percentage_increase, 0)
        ...
        for strategy_name in strategy_names:
            if self.open_positions[strategy_name]['type'] is not None:
                bid_price = bid_prices[-1]
                ask_price = ask_prices[-1]
                profit = self.close_position(
                    strategy_name, bid_price, ask_price)
                self.total_profit_or_loss[strategy_name] += profit
                self.trade_returns[strategy_name].append(profit)
        ...
        strategy_names = ['mean_reversion', 'pure_forcasting',
                          'ensemble_with_llm_mean_reversion']
        for strategy_name in strategy_names:
            self.sharpe_ratios[strategy_name] = self.calculate_sharpe_ratios(
                strategy_name)
