...
def run_sl_based_trading_strategy(model_config):
    ...
    if model_config.MODEL_NAME == 'bilstm':
        ...
        test_series, test_dates, test_bid_prices, test_ask_prices, test_with_prompt, test_without_prompt = predictor.split_and_scale_data()
    ...
    trading_strategy.simulate_trading_with_strategies(true_values, predicted_values, test_bid_prices, test_ask_prices, test_with_prompt, enable_transaction_costs=model_config.ENABLE_TRANSACTION_COSTS, hold_position=model_config.HOLD_POSITION)
    ...
    trading_strategy = TradingStrategy(model_config.WALLET_A, model_config.WALLET_B)
    trading_strategy.simulate_trading_with_strategies(true_values, predicted_values, test_bid_prices, test_ask_prices, test_with_prompt, use_kelly=False, enable_transaction_costs=model_config.ENABLE_TRANSACTION_COSTS, hold_position=model_config.HOLD_POSITION)
    mean_reversion_profit.extend(trading_strategy.trade_returns["mean_reversion"])
    trend_profit.extend(trading_strategy.trade_returns["trend"])
    forecasting_profit.extend(trading_strategy.trade_returns["pure_forcasting"])
    hybrid_mean_reversion_profit.extend(trading_strategy.trade_returns["hybrid_mean_reversion"])
    hybrid_trend_profit.extend(trading_strategy.trade_returns["hybrid_trend"])
    ensemble_profit.extend(trading_strategy.trade_returns["ensemble"])
    # mean_reversion_profit_per_trade_val = trading_strategy.trade_returns["mean_reversion"] / trading_strategy.num_trades["mean_reversion"]
    # trend_profit_per_trade_val = trading_strategy.trade_returns["trend"] / trading_strategy.num_trades["trend"]
    # forecasting_profit_per_trade_val = trading_strategy.trade_returns["pure_forcasting"] / trading_strategy.num_trades["pure_forcasting"]
    # hybrid_mean_reversion_profit_per_trade_val = trading_strategy.total_profit_or_loss["hybrid_mean_reversion"] / trading_strategy.num_trades["hybrid_mean_reversion"]
    # hybrid_trend_profit_per_trade_val = trading_strategy.total_profit_or_loss["hybrid_trend"] / trading_strategy.num_trades["hybrid_trend"]
    # ensemble_profit_per_trade_val = trading_strategy.total_profit_or_loss["ensemble"] / trading_strategy.num_trades["ensemble"]
    # mean_reversion_profit_per_trade.append(mean_reversion_profit_per_trade_val)
