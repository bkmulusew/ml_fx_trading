...
def plot_prediction_comparison(true_values, predicted_values, model_name):
    ...
    plt.ylabel('Ratio')
    plt.legend()
    plt.savefig(f'true_vs_predicted_{model_name}.png', dpi=300, bbox_inches='tight')
    plt.clf()
...
def run_sl_based_trading_strategy(model_name, model_config, trade_thresholds):
    ...
    for trade_thresold in trade_thresholds:
        ...
        for date_key, values in chunked_values.items():
            ...
            trading_strategy.simulate_trading_with_strategies(values['true_values'], values['predicted_values'], values['bid_price'], values['ask_price'], values["with_prompt"], enable_transaction_costs=model_config.ENABLE_TRANSACTION_COSTS)
            ...
            ensemble_with_llm_mean_reversion_sharpe_ratios.append(trading_strategy.sharpe_ratios["ensemble_with_llm_mean_reversion"])
    ...
    plt.plot(cumulative_forecasting_profit_per_trade, color='red', label='Cumulative Forecasting Profit Per Trade TCN')
    ...
    plt.legend()
    plt.savefig('cumulative_profits_per_trade_kelly.png', dpi=300, bbox_inches='tight')
    ...
    plt.legend()
    plt.savefig('mean_reversion_vs_forecasting_coeffs_kelly.png', dpi=300, bbox_inches='tight')

    plt.clf()

    plt.plot(cumulative_mean_reversion_profit, color='purple', label='Cumulative Mean Reversion Profit')
    plt.plot(cumulative_forecasting_profit, color='red', label='Cumulative Forecasting Profit TCN')
    ...
    plt.plot(forecasting_profit_per_trade, color = 'red', label = 'Forcasting Profit Per Trade TCN')
    ...
    plt.plot(forecasting_sharpe_ratios, color='red', label='Forecasting Sharpe Ratio TCN')
    ...
    for trade_thresold in trade_thresholds:
        ...
        for date_key, values in chunked_values.items():
            trading_strategy = TradingStrategy(model_config.WALLET_A, model_config.WALLET_B, model_config.FRAC_KELLY, trade_thresold)
            trading_strategy.simulate_trading_with_strategies(values['true_values'], values['predicted_values'], values['bid_price'], values['ask_price'], values["with_prompt"], use_kelly=False, enable_transaction_costs=model_config.ENABLE_TRANSACTION_COSTS)
            mean_reversion_profit.append(trading_strategy.total_profit_or_loss["mean_reversion"])
            forecasting_profit.append(trading_strategy.total_profit_or_loss["pure_forcasting"])
            ensemble_with_llm_mean_reversion_profit.append(trading_strategy.total_profit_or_loss["ensemble_with_llm_mean_reversion"])
            mean_reversion_profit_per_trade_val = trading_strategy.total_profit_or_loss["mean_reversion"] / trading_strategy.num_trades["mean_reversion"]
    ...
    plt.ylabel('Cumulative Profit Per Trade')
    plt.legend()
    plt.savefig('cumulative_profits_per_trade_fixed.png', dpi=300, bbox_inches='tight')
    ...
    plt.plot(forecasting_coeffs, color='red', label='Forecasting Coefficient')
    plt.plot(llm_sentiment_coeffs, color='green', label='LLM Coefficient')
    plt.title('Mean Reversion, Forecasting, and LLM Coefficients Using Fixed Postion Size')
    plt.xlabel('Observation')
    plt.ylabel('Coefficient')
    plt.legend()
    plt.savefig('mean_reversion_vs_forecasting_coeffs_fixed.png', dpi=300, bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--enable_transaction_costs", type=bool, default=False, help="Enable transaction costs.")
    parser.add_argument(
        "--trade_thresholds", 
        type=str, 
        default="0", 
